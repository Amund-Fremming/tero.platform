...
mod tests {
    ...
    use crate::{
        models::{app_state::AppState, game_base::GameTable},
    ...
    async fn max_limit_keys() {
        ...
        for num in 0..10_000 {
            let word = vault.create_key(state.get_pool(), GameTable::Quiz).unwrap();
            println!("{} - {}", num + 1, word)
        }

        let result = vault.create_key(state.get_pool(), GameTable::Quiz);
    ...
    async fn test_concurrent_key_creation() {
        ...
        for i in 0..10001 {
            ...
            let handle = tokio::spawn(async move {
                ...
                match vault.create_key(state_clone.get_pool(), GameTable::Quiz) {
                    Ok(key) => {
