...
use crate::{
    common::{
        ...
        auth::Claims,
        error::ServerError,
        services::gs_client::{InteractiveGameResponse, JoinGameResponse},
    ...
    features::{
        game::{
            models::{
                ...
                InitiateGameRequest, ImposterSession, InteractiveEnvelope, QuizSession,
                ResponseWrapper, SavedGamesPageQuery, SpinSession,
            },
            repository::{
                create_game_base, create_imposter_game, create_quiz_game, create_spin_game,
                delete_game as delete_game_from_db, delete_saved_game, get_game_page,
                get_quiz_game_by_id, get_saved_games_page, get_spin_game_by_id, save_game,
                sync_and_increment_times_played, take_random_game,
            },
        },
        user::models::{Permission, SubjectId},
    },
};
...
async fn delete_game(
    ...
    let deleted_game = delete_game_from_db(state.get_pool(), game_id).await?;
