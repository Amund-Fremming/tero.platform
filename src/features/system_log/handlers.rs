...
use crate::{
    ...
    features::{system_log::repository, user::models::{Permission, SubjectId}},
};
...
async fn get_system_log_page(
    ...
    let page = repository::get_system_log_page(state.get_pool(), query).await?;
...
async fn get_log_category_count(
    ...
    let counts = repository::get_log_category_count(state.get_pool()).await?;
