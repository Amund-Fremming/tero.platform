...
use crate::{
    common::{app_state::AppState, auth::Claims, error::ServerError},
    features::{
        game_tip::{models::{CreateGameTipRequest, GameTipPageQuery}, repository},
...
async fn create_game_tip(
    ...
    let tip_id = repository::create_game_tip(state.get_pool(), &request).await?;
...
async fn get_game_tips_admin(
    ...
    let page = repository::get_game_tips_page(state.get_pool(), query.page_num).await?;
